...
export default class RetslyHotspotQuery extends Action {
  constructor(address) {
    ...
    let params = `access_token=${process.env.RETSLY_TOKEN}&near=${encodeURIComponent(address)}&radius=1km&documentDate.gt=${oldestDate}`;
  ...
  handleSuccess(response) {
    response.json().then((actualBody) => {
      this.hotspotTotal = actualBody.total;
